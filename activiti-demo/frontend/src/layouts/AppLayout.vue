  ...
  <div class="app-shell">
    ...
    <header class="shell-topbar">
      <div class="shell-topbar-left">
        ...
        <strong class="shell-brand">OpenActiviti</strong>
...
import { clearCurrentUser, clearToken, getCurrentUser, getToken, http, listenAuthChanged, saveCurrentUser } from '../api/http'
...
const menuCatalog = [
  ...
  },
  {
    ...
    icon: 'process',
    items: [
      { name: '流程设计', path: '/designer', permission: 'process:design' },
      { name: '发起流程', path: '/leave', permission: 'leave:submit' },
      { name: '待办审批', path: '/tasks', permission: 'task:approve' },
      { name: '流程编排', path: '/orchestration', permission: 'orchestration:manage' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: '业务页目录', path: '/runtime-catalog', permission: 'runtime:catalog:manage' }
    ]
  }
]
...
function resolveAccessiblePath() {
  ...
  if (menus.value.length) {
    return menus.value[0].path || '/dashboard'
  }
...
async function syncSessionState(options = {}) {
  const nextUser = options.user ?? getCurrentUser()
  currentUser.value = nextUser
  if (!getToken() || !nextUser) {
    menus.value = []
    syncExpandedGroups()
    return
  }
  ...
  syncExpandedGroups()
  if (route.meta?.permission && !canAccessMenu(route.path, route.meta.permission)) {
    const fallbackPath = resolveAccessiblePath()
    if (fallbackPath && fallbackPath !== route.path) {
      router.replace({ path: fallbackPath, query: { denied: '1' } })
    }
  }
}
...
onMounted(async () => {
  stopAuthListener = listenAuthChanged((user) => {
