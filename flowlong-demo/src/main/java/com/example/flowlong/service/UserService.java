...
import com.example.flowlong.repository.UserRepository;
...
public class UserService {
    ...
    private final Object seedUsersLock = new Object();
    ...
    public PageResult<User> queryUsers(UserQueryRequest query) {
        ensureSeedUsers();
        List<User> items = relationLoader.attachRoles(userRepository.findPageByCondition(query));
        long total = userRepository.countByCondition(query);
        return PageBuilder.build(query, items, total);
    }
    ...
    public User updateUser(Long id, String nickname, String email, String phone, Integer status, List<Long> roleIds) {
    ...
    public void deleteUser(Long id) {
        ...
        if ("admin".equalsIgnoreCase(user.getUsername())) {
            throw new RuntimeException("默认管理员不允许删除");
        }
        User currentUser = getCurrentUser();
        if (currentUser != null && id.equals(currentUser.getId())) {
            throw new RuntimeException("当前登录用户不允许删除自己");
        }
