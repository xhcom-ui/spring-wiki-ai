...
let viewer
let previousMarkers = []
let removeElementClickListener = null
...
function clearMarkers() {
  ...
  for (const item of previousMarkers) {
    try {
      ...
      // ignore missing element markers
    }
  }
  previousMarkers = []
}
...
function applyMarkers() {
  ...
  const nextMarkers = [
    ...
    ...markerEntries(props.selectedNodeId ? [props.selectedNodeId] : [], 'trace-selected')
  ]
  for (const item of nextMarkers) {
    try {
      ...
      // ignore markers for elements that do not exist in the diagram
    }
  }
}
...
watch(
  () => [props.highlightedNodeIds, props.completedActivityIds, props.activeActivityIds, props.gatewayIds, props.currentActivityId, props.selectedNodeId, props.sequenceFlowIds],
  () => {
    applyMarkers()
  },
  { deep: true }
...
onMounted(async () => {
  try {
    ...
    const handleElementClick = (event) => {
      ...
      const elementId = businessObject?.id || element?.id
      if (!elementId) {
        return
      }
      emit('node-click', {
    ...
    removeElementClickListener = () => eventBus.off('element.click', handleElementClick)
    if (props.xml) {
      await importXml(props.xml)
    }
  } catch (error) {
    ready.value = false
    loadError.value = error?.message || 'BPMN 轨迹图初始化失败'
  }
})
...
:deep(.trace-selected .djs-visual > :nth-child(1)) {
  stroke: #1d4f91 !important;
  stroke-width: 6px !important;
  filter: drop-shadow(0 0 12px rgba(29, 79, 145, 0.24));
}
