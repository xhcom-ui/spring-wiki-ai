    ...
    <div v-else class="main-page">
      ...
      <div class="content">
        ...
        <div v-if="currentTab === 'process'" class="section">
          ...
          <div class="form-group">
            <label>流程定义</label>
            <textarea v-model="processDefinition" placeholder="请输入流程定义 XML" rows="10"></textarea>
          </div>
          <button @click="deployProcessFromXml" :disabled="loading || !processName || !processDefinition">
            {{ loading ? '部署中...' : '部署流程' }}
          </button>
        </div>
...
<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
  ...
  <process id="leave-process" name="请假流程" isExecutable="true">
    ...
    <userTask id="submitTask" name="提交申请" camunda:assignee="${this.leaveForm.applicant}" />
    <userTask id="deptTask" name="部门经理审批" camunda:assignee="${this.leaveForm.deptManager}" />
    <userTask id="generalTask" name="总经理审批" camunda:assignee="${this.leaveForm.generalManager}" />
    ...
    async deployProcess() {
      ...
      try {
        const { xml } = await this.bpmnModeler.saveXML({ format: true });
        const response = await axios.post('/api/flowable/deploy', {
    ...
    async submitLeave() {
      ...
      try {
        const response = await axios.post('/api/flowable/leave/start', this.leaveForm);
    ...
    async getTasks() {
      ...
      try {
        const response = await axios.get(`/api/flowable/tasks/${this.assignee}`);
    ...
    async approveTask(taskId, processInstanceId) {
      ...
      try {
        await axios.post('/api/flowable/task/complete', {
          taskId,
          variables: {
            approved: true
          },
    ...
    async getLeaves() {
      ...
      try {
        const response = await axios.get('/api/flowable/leaves');
