...
function defaultUserTask(name = '审批人', assignment = '') {
  return {
...
function summarizeCopyTask(node) {
  const properties = node?.properties || {}
  const selectedUsers = Array.isArray(properties.copyUserList)
    ? properties.copyUserList.map((item) => safeText(item?.name, item?.id)).filter(Boolean)
    : []
  const users = selectedUsers.join('、') || safeText(properties.copyUsers)
  const labels = []
  if (users) {
    labels.push(`抄送 ${users}`)
  }
  if (properties.allowInitiatorSelect) {
    labels.push('发起人可自选')
  }
  return labels.join(' · ') || '待配置抄送人'
}
...
function ensureNode(node) {
  ...
  if (node.nodeType === 'serial') {
    ...
    node.conditionNodes.forEach((item, index) => {
      ...
      item.properties.isDefault = Boolean(item.properties.isDefault || item.default)
      item.default = item.properties.isDefault
      item.nodeName = safeText(item.nodeName, `条件 ${index + 1}`)
      item.value = safeText(item.value, item.properties.conditionSummary || (item.properties.isDefault ? '默认分支' : ''))
    })
    if (!node.conditionNodes.some((item) => item.properties?.isDefault)) {
      const last = node.conditionNodes[node.conditionNodes.length - 1]
      last.properties.isDefault = true
      last.default = true
      last.value = safeText(last.value, '默认分支')
    }
  }
  ...
  if (node.nodeType === 'between') {
    ...
    node.value = safeText(node.value, summarizeUserTask(node))
  }
  if (node.nodeType === 'copy') {
    node.properties.copyUsers = safeText(node.properties.copyUsers)
    node.properties.copyUserList = Array.isArray(node.properties.copyUserList)
      ? node.properties.copyUserList
          .map((item) => ({
            id: safeText(item?.id),
      ...
      : []
    node.properties.allowInitiatorSelect = Boolean(node.properties.allowInitiatorSelect)
    node.value = safeText(node.value, summarizeCopyTask(node))
  }
  if (node.nodeType === 'start') {
    node.properties.initiatorLabel = safeText(node.properties.initiatorLabel, '发起人')
    node.properties.initiatorUsers = safeText(node.properties.initiatorUsers)
    node.properties.initiatorUserList = Array.isArray(node.properties.initiatorUserList)
      ? node.properties.initiatorUserList
          .map((item) => ({
            id: safeText(item?.id),
            name: safeText(item?.name, item?.id)
          }))
    ...
    node.properties.formLabel = safeText(node.properties.formLabel)
    node.properties.pageLabel = safeText(node.properties.pageLabel)
    node.value = safeText(node.value, safeText(node.properties.initiatorLabel, '发起人'))
  }
...
export function createDefaultCustomSchema({ processName = '新建流程' } = {}) {
  return ensureNode({
    ...
    childNode: {
      ...defaultUserTask(processName.includes('请假') ? '部门经理审批' : '审批人', processName.includes('请假') ? '${deptManager}' : ''),
      childNode: {
        nodeId: createId('end'),
        nodeType: 'end',
        nodeName: '结束',
        ...
        conditionNodes: [
          {
            ...
            value: 'approved = true',
            properties: {
              conditionSummary: 'approved = true',
...
export function updateDerivedNodeValue(node) {
  ...
  } else if (node.nodeType === 'copy') {
    node.value = summarizeCopyTask(node)
  } else if (node.nodeType === 'start') {
    node.value = safeText(node.properties?.initiatorLabel, '发起人')
  } else if (node.nodeType === 'serial-node') {
    const properties = node.properties || {}
    node.value = properties.isDefault
      ? '默认分支'
      : safeText(properties.conditionSummary, safeText(properties.conditionExpression, '请设置条件'))
    node.default = Boolean(properties.isDefault)
  }
}

function createSequenceId(prefix = 'Flow') {
...
function createDiagramShape(diagram, id, type, x, y) {
  ...
  diagram.bounds[id] = { x, y, width, height, centerX: x + width / 2, centerY: y + height / 2, type }
...
function createUserTaskExtensions(node) {
  const properties = node.properties || {}
  const listeners = Array.isArray(properties.listeners) ? properties.listeners.filter((item) => safeText(item.listenerType) && safeText(item.listenerPath)) : []
  if (!listeners.length) {
    return ''
  }
  const listenerXml = listeners
    .map((item) => `<flowable:taskListener event="${escapeXml(item.listenerType)}" class="${escapeXml(item.listenerPath)}" />`)
    .join('')
  return `<bpmn:extensionElements>${listenerXml}</bpmn:extensionElements>`
}

function createNodeElement(diagram, node, x, y) {
  if (node.nodeType === 'start') {
    diagram.elements.push(`<bpmn:startEvent id="${escapeXml(node.nodeId)}" name="${escapeXml(node.nodeName)}" />`)
  ...
  if (node.nodeType === 'between') {
    ...
    const documentation = [safeText(properties.pageLabel), safeText(properties.formLabel)]
      .filter(Boolean)
      .join(' / ')
    const content = []
    if (documentation) {
    ...
    const extensions = createUserTaskExtensions(node)
    if (extensions) {
...
function renderNodeChain(node, x, y, diagram) {
  ...
  if (node.nodeType === 'copy') {
    const childResult = renderNodeChain(node.childNode, x, y, diagram)
    if (childResult.startId) {
      return childResult
    }
    return { startId: '', endId: '', maxX: x, minY: y, maxY: y }
  }
...
function renderExclusiveBranch(node, x, y, diagram) {
  ...
  const offsetBase = (branches.length - 1) / 2
  const branchResults = []
  ...
  branches.forEach((branch, index) => {
    const branchY = y + (index - offsetBase) * 170
    let branchResult
    if (branch.childNode) {
      branchResult = renderNodeChain(branch.childNode, x + 220, branchY, diagram)
    } else {
      branchResult = { startId: '', endId: '', maxX: x + 220, minY: branchY, maxY: branchY }
    }
    branchResults.push({ branch, branchY, branchResult })
...
function renderParallelBranch(node, x, y, diagram) {
  ...
  const branches = Array.isArray(node.conditionNodes) && node.conditionNodes.length ? node.conditionNodes : [defaultUserTask('并行审批 1'), defaultUserTask('并行审批 2')]
  const offsetBase = (branches.length - 1) / 2
  const branchResults = []
  let maxBranchX = x + 220
  let minY = y
...
export function buildCustomBpmnXml(schema, { processId = 'process', processName = '新建流程' } = {}) {
  const normalizedSchema = normalizeCustomSchema(schema, { processName })
  const diagram = {
    elements: [],
    sequenceFlows: [],
    bounds: {}
  }
  renderNodeChain(normalizedSchema, 80, 220, diagram)
