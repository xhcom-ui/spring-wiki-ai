					...
					<ul>
						<el-scrollbar>
							<li v-for="(item, index) in selected" :key="item.id">
								...
								<span class="delete">
									<el-button type="danger" circle size="small" @click="deleteSelected(index)">
...
<script>
	import config from '../../../config/flowlong-workflow.js'
	...
	export default {
		...
		methods: {
			upsertSelected(item){
				...
				const existed = this.selected.findIndex((selectedItem) => selectedItem.id === item.id)
				if(existed >= 0){
					this.selected.splice(existed, 1, item)
					return
				}
